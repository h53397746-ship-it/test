...
# IP Rotation via Browserless regions
BROWSERLESS_REGIONS = [
    'production-sfo',  # San Francisco
    'production-lon',  # London
    'production-syd',  # Sydney
]

region_rotation_index = 0

def get_next_browserless_region():
    """Rotate through Browserless regions for different IPs"""
    global region_rotation_index
    
    if not CONFIG["IP_ROTATION_ENABLED"]:
        return 'production-sfo'
    ...
    async with async_playwright() as p:
        ...
        try:
            if CONFIG["RUN_LOCAL"]:
                ...
                # Direct WebSocket connection to Browserless (no BQL)
                browserless_url = f"wss://{region}.browserless.io/chromium/playwright?token={CONFIG['BROWSERLESS_API_KEY']}&timeout={CONFIG['BROWSERLESS_TIMEOUT']}&stealth=true"
                ...
                try:
                    ...
                    if "429" in error_msg:
                        ...
                        return {"error": f"Connection failed: {error_msg}"}
...
def home():
    return jsonify({
        ...
        "endpoints": {
            "/hrkXstripe": "Main endpoint",
            "/status": "Status check",
            "/ip-stats": "IP rotation stats"
        }
    })
...
if __name__ == '__main__':
    ...
    print(f"[IP ROTATION] Multi-region ({len(BROWSERLESS_REGIONS)} regions)")
    
    if not CONFIG['RUN_LOCAL'] and not CONFIG.get('BROWSERLESS_API_KEY'):
        print("[WARNING] BROWSERLESS_API_KEY not set!")
    
    print("\n[FEATURES]")
    print("  ✓ Browserless WebSocket (no BQL)")
    print("  ✓ IP rotation via regions")
    print(f"  ✓ Regions: {', '.join(BROWSERLESS_REGIONS)}")
    print("  ✓ Stealth mode")
    print("  ✓ Random fingerprints")
